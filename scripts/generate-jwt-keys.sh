...
echo "🔐 Generating ECDSA P-256 keys for JWT signing..."
echo ""
...
PRIVATE_KEY_SINGLE_LINE=$(cat "$KEYS_DIR/jwt-private-key.pem" | tr '\n' '\\n')
PUBLIC_KEY_SINGLE_LINE=$(cat "$KEYS_DIR/jwt-public-key.pem" | tr '\n' '\\n')
...
if grep -q "^JWT_PRIVATE_KEY=" "$ENV_FILE"; then
  echo "⚠️  JWT_PRIVATE_KEY already exists in .env, updating..."
...
if grep -q "^JWT_PUBLIC_KEY=" "$ENV_FILE"; then
  echo "⚠️  JWT_PUBLIC_KEY already exists in .env, updating..."
...
echo "JWT_PRIVATE_KEY=\"$PRIVATE_KEY_SINGLE_LINE\"" >> "$ENV_FILE"
echo "JWT_PUBLIC_KEY=\"$PUBLIC_KEY_SINGLE_LINE\"" >> "$ENV_FILE"
...
echo "✅ Keys and JWT configuration added to .env file!"
echo ""
echo "⚠️  IMPORTANT:"
